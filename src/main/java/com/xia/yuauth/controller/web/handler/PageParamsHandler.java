    ...
    public HttpInputMessage beforeBodyRead(HttpInputMessage inputMessage, MethodParameter parameter, Type targetType,
        ...
        if (isVerify) {
            if (targetType == PageParams.class) {
                ...
                PageParams pageParams = JSON.toJavaObject(JSON.parseObject(json), PageParams.class);
                verifyRequiredPageParams(pageParams);
            }

            Result<Object> objectResult = new Result<>().withCode(ResultStatusEnum.PARAMS_NOT_MATCHES.getCode());
            throw new ServiceException(objectResult);
        }
        return inputMessage;
    }
